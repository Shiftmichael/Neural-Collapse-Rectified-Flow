...
from utils_reflow.utils import seed_everything, generate_image_pairs, generate_real_image_pairs, mix_and_shuffle_batches
...
def train(rank, total_num_gpus, argv):
    ...
    for reflow_step in range(FLAGS.reflow_times):
        ...
        # real_sampler = DistributedSampler(real_data) if FLAGS.parallel else None
        real_sampler = RandomSampler(real_data)
        if FLAGS.parallel:
            ...
            realdataloader = torch.utils.data.DataLoader(
                real_data,
                batch_size=batch_size_per_gpu,
        ...
        real_datalooper = infiniteloop(realdataloader)
            ...
            for epoch in epoch_pbar:
                ...
                with trange(steps_per_epoch, dynamic_ncols=True) as step_pbar:
                    for step in step_pbar:
                        ...
                        batch_generated = next(datalooper)
                        batch_real = next(real_datalooper)
